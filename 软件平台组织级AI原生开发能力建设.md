...
Lowell 纺织厂的三次演进：从左至右依次为 1890 年蒸汽动力厂房、1900 年电力驱动厂房，以及 1920 年从零重建的电气化装配线厂房。
...
- **流程错配**：编码提速但评审、联调、测试、现场交付调试等过程未能提速，形成新的瓶颈。
...
![[Pasted image 20260614224338.png]]
因此，约15%-35%的个体提效容易停留在局部，而无法转化为组织级端到端效率提升。



![[Pasted image 20260614222301.png]]
...
![[Pasted image 20260614225355.png]]
...
更深一层看，个体 10x 不等于组织 10x。前面的1890 年代纺织厂的例子：工厂换上了电动机，但没有重新设计厂房布局和生产流程，导致电气化后 30 年产出几乎零增长——**我们用AI相当于也是换上了"电动机"，但"工厂"还没有重新设计**。组织级智能不会随个体使用 AI 工具自然涌现，它需要在**协调、信号提取、确定性保障**三个支柱上构建制度体系。这意味着：**流程工程（Process Engineering）** 才是核心——不是编码产品，而是把企业流程编码进 Agent 并推动组织变革管理，其中**领域专业知识比软件工程专业知识更关键**。目标必须从"节省时间"升维到"规模化价值"——否则"AI 生码率"陷阱不可避免。正如业界所言：问任何 CEO 首要目标是降本还是增收，几乎所有人都会说是收入；但几乎所有 AI 产品都在卖"省时、少人"——这中间的鸿沟，就是组织级能力建设的意义所在。

---

### 务实路径：建立组织级能力基线与渐进式演进

首先，我们须认清当前仍处于**个人普及向组织建设过渡**的阶段。**高生产力个体无法自动组成高生产力组织**。我们的重心已经不该是编码本身，而是**组织运转方式**——正如前述，**流程工程（Process Engineering）** 才是核心，其中领域专业知识比软件工程专业知识更关键，目标必须从"节省时间"升维到"规模化价值"。

其次，我们应该积极推动组织和人才的转型，逐步构建新的平台、度量、实践三大智能工程支柱。在度量体系上，必须坚决摒弃"AI 生码率"等过程指标陷阱，转向更能指向E2E效率或者质量结果的相关指标，引导开发者保持健康的使用行为。

最后，在架构演进上，应采纳渐进式路径，按需引入上下文增强、质量门禁等，避免过早陷入 Agent 模式的复合错误率陷阱。当然，在模型能力达到临界点前，**半自动人机协作（60% 环节使用 AI，核心决策保留人类判断）** 是更具性价比的选择。

我们的目标是让软件开发过程的AI应用跨过个人和小团队的阶段，走向集体，走向我们整个组织。
![[Pasted image 20260614225123.png]]
...
不过，模型和基础算力等方面的问题更多有赖于采购和推动其他部门解决，不是我们本文讨论的重点。各团队遇到的基础算力相关的问题如果解决不了，可以反馈给我们的专项项目经理黄宁，我们一起继续来推动。
...
![[Pasted image 20260614224525.png]]
...
##### 垃圾回收式技术债务管理
摒弃传统的"集中清理"模式，采用**垃圾回收式技术债务管理**。将"黄金原则"编码入仓库，部署背景 Agent 定期扫描代码偏离情况、做架构走查、更新质量评级，并自动开启定向重构 PR，持续进行小额度自动化重构，防止冗余代码堆积。另外，考核上尽量避免"AI 生码率"这种虚荣指标（也是我在研发中心和软研一再反对的，这种指标观察即可）。
...
##### 上下文工程与渐进式信息暴露
   ...
   - **上下文工程**：将上下文管理从"战术优化"升级为"基础架构关切"（具体做法详见知识工程章节）。
...
上游需求与设计质量不足，会沿交付链路逐级放大——下游的 AI 生成质量因此系统性下降。
...
假设上游没有定义某种极端异常分支（比如某存储产品和云存储接口类似但缺少关键容错特性，ICC 按照云存储逻辑对接会有丢数据风险）。人类程序员在写代码时会卡住，从而倒逼产品经理澄清需求或者架构师拉通方案；但 AI 会基于概率，自信地编造一个"看似合理但业务错误"的默认处理逻辑（例如直接返回成功或静默失败）。这段代码能跑通，单元测试也能过，但核心业务逻辑全错。由于 AI 生成代码极快，这种"**方向性垃圾**"会被大规模、快速地转化为生产环境中的**代码即负债**（详见前述"AI 生码率陷阱与代码即负债"）。
...
##### 知识结构化：场景化知识库与渐进式信息暴露
   ...
   - 面对海量的存量工程知识，采用**渐进式信息暴露**策略，为 Agent 提供"小而稳定的入口点"，按需深入加载。

##### 维护后台化：部署背景Agent 定期、按需维护更新
   - 知识工程不是一次性工作。如前述 AI 执行偏离章节应对所述，部署背景 Agent 定期扫描代码与文档的偏离情况；知识更新与校验机制应纳入该常态化流程。
...
##### 能力培养：本质是"投入分摊方式"问题
   - 这不是态度问题，是结构性的——任何新技术学习都需要前期投入产出比为负的阶段，而 KPI/排期通常按短期产出考核，导致个体和团队没有"合法"的时间做这件事，只能"挤"或"不做"。
...
##### 经验复制不是直接照搬
   - 头部企业的实践（比如某种 Agent 架构、某套 Prompt 工程方法论）是在特定场景、数据规模、技术栈下打磨出来的，直接迁移会因为业务场景、数据特点、团队能力错配而失败。但这不代表这些实践没有价值，而是缺少一个"提炼通用原则—结合自身场景再设计"的转译过程。
...
各开发团队反馈："1) 工具系统多，比较离散；2) 提效数据缺乏统一评估维度；3) 全工程维度提效分析尚缺失；4) 人机协作过程如黑箱，不可见"

---

#### 现象本质

- 大华研发体系开发环节多，工具和系统离散。AI 来之前如此，AI 来之后，因为工具开发速率在理论上可以无限快，工具似乎会更加繁荣。越来越多的工具和平台到底如何能给开发者一站式体验，如何能给出标准的一致性的度量数据？
...
- 一站式体验的落点不在"减少工具数量",而在**汇聚层/聚合视图**:在工具之上建一层轻量聚合,让开发者从一个入口感知全局状态,底层工具仍各自独立演进。举个例子，如果开发者用的是CodeBuddy，我们的慧码平台（插件形态）把所有的rule/skill标准体系、外部系统对接、度量采集内置掉，集成给CodeBuddy，开发者只感知CodeBuddy。如果开发者自己做了个工作台，我们的慧码插件被他集成，则在体验有差异之外，一样有一致性的技术规范；
- 该协议应作为新工具/Agent接入的"准入门槛"之一,从源头控制离散度的增长,而非事后治理存量；
- 同时鼓励大家共创工具，开源社区方式推进。但需要HR协助做好激励机制，因为这部分都是在自身业务之外的投入。

另外，必须指出的是，虽然我们大华过去的系统多、工具多，但现在我们反而认为这个问题可以不再显得那么严重了——这些系统可以快速封装为 MCP Server，以命令行形式供 Agent 调用，从而复用现有资产。
...
意义:防止度量体系隐含"全自动=更好"的错误导向,从数据层面支持"半自动模式比盲目全自动更具性价比"的判断,同**时**避免"用节省时间生产更多垃圾代码"的认知陷阱——这一点直接呼应了反馈二中"效率指标必须与质量指标联动"的要求,且给出了具体的联动方式。
...
- 端到端数据链路打通后,首要验证问题应是：**个人层面 15%-35% 的潜在提效（见前述原因分析），是否在组织层面被验证税、返工、流程瓶颈抵消甚至转为负值**
...
- **知识工程**（K for Knowledge）面向AI准备的知识工程体系
- **流程引擎**（E for Engine）人和AI协作的开发流程和对应工具
- **评估治理**（E for Evaluation）Agent开发的质量/安全/合规/可用性等各方面的评估治理体系
- **组织人才**（P for People）新的开发组织形式和人的能力成长要求
...
- 全栈（或者"半栈"，总之是尽可能覆盖更多技能或环节）能力的组织重构需试点推进
- AI 使用效率需纳入正式度量体系
...
| 推行 全栈组织重构试点                                        | P    | 提升小团队作战能力                       |
| 领域 Harness 治理规范和工程实践落地                             | E    | 政府、企业、基础、海外、西安、成都等各团队均落地自适应流程规范 |

**年度关键成果目标**：
-  知识工程：跨领域知识体系建立，分级分类的知识库体系全面运作
-  流程引擎：Skill 生态成熟，构建Harness 体系支撑 人-Agent 协同
-  评估治理：确定性裁判体系全面部署，探索Token 消耗效率等评估方式
-  组织人才：新角色落地，CoE运作。研发中心所要求的全栈开发占比 > 10%（通过事实上的任务输出结果可证明）
...
- **one runtime**：一个慧码 Agent Runtime。解决 spec 规范沉淀、过程个人度量数据采集、外部系统对接等功能
...
#### LLM wiki和RAG探索
新的知识库构建：
- LLM wiki。面向个人和团队，提供LLM wiki能力。重点在于知识图谱关联/新知识发现，实现摄入-编译-探索-检查的闭环。
- RAG。统一建设。提供向量检索增强。
（待实施。LLM wiki架构在团队内试用）
...
![[Pasted image 20260614152825.png|627]]
整个看板的体系说明：基础算力、业务视角算力和工具使用、AI数字资产体系、效能数据。



![[Pasted image 20260614152842.png]]
1、通过用户数、工具数、token用量 反应本地化算力总体使用情况
2、通过并发与等待请求、API平均时延反应 用户体验情况
3、通过系统负载和上述指标比对，反应本地算力和用户的匹配情况


![[Pasted image 20260614153135.png]]
1、主体呈现各行业线的工具使用情况（包含token、限流时长、并发、时延）
3、针对工具的异常情况进行告警（如限流时间过长，token用量过多等）
...
1、重点呈现软研各下钻组织的token用量、限流、时延、请求成功率等情况
2、通过异常告警，能快速发现用量异常、时延异常的情况，针对性的进行上下文或算力本身的优化
3、所有组织均支持下钻到个人，查看个人的详细使用情况及趋势


![[Pasted image 20260614153203.png]]
1、呈现Skill、MCP、Rule、Agent、工具集等资产的分布情况
2、呈现资产的新增趋势
3、呈现资产各部门的分布情况

![[Pasted image 20260614153208.png]]
1、呈现高影响力以及高活跃度资产，并推荐给用户
2、同时按组织对高质量、高维护性、僵尸资产进行晾晒
...
通过打开软研各业务线各产品、版本的计划、需求、开发、测试等维度的详细数据，形成多维度的效能评价体系。

后续还需要：
- 通过效能看板识别跨团队瓶颈，特别是等待时间异常，自动化能力瓶颈，推动对应团队分析。
- ==除了工时之外，token消耗最终也要能进入到产品、版本、组件、项目中去==。


这些不是终局，我们做产品，最终回答的还是产品面世的速度和质量：
![[Pasted image 20260614160337.png]]
而结合市场数据，ITR数据，定制数据，萤火虫数据，组件血缘，TB项目过程数据，AI原生开发度量数据，最终我们是应该可以建立一条人机协同、市场研发端到端的价值溯游回路的。
...
- **阶段二：孵化期（Incubating）**
    - **规则：** 工具在跨团队（至少两个以上业务线）产生价值。需要在社区登记，平台工程团队开始介入，协助提供标准化的部署资源（如容器集群、网关接入）。要求完善 API 文档和使用手册（可沉淀至内部 LLM-Wiki 知识库）。
...
工作方式上，核心思路是前述"垃圾回收式"技术债务管理（详见 AI 执行偏离章节应对）在评估治理层面的落地：把团队认可的"黄金原则"（如命名规范、模块边界约定、禁止的反模式）编码为可被Agent识别和执行的规则，部署常驻Agent定期扫描、识别偏离并更新质量评级，结果同步至看板。能力成熟后，常驻Agent可进一步升级为主动治理者。

针对识别出的偏离，自动生成定向重构PR，以小批量、低风险的方式持续修复。这种方式的核心优势是把技术债务管理从"周期性的痛苦项目"变成"系统的日常代谢"，代码库始终保持可控健康水位，避免冗余代码和架构腐化随AI开发速度加快而加速累积。
...
去年（2025年）我们讲软件工程化专项，提的是三大效能支柱；今年我们进入AI原生开发，构建的是KEEP四维度的成熟度模型。它们之间不是替代关系，而是一种延续。
![[Pasted image 20260614211104.png]]
