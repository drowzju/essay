...
- 软件开发组织应用 AI到生产过程，从个人工具使用走向组织运转，应该有哪些关键工作？流程建设重点和工作重心是什么？
- 在大华软研，应用 AI提效，今天为止我们有哪些问题？
- 结合业界的实践经验和我们遇到的问题，如何评估我们的组织级 AI 应用能力？如何明确我们后续的方向和行动？
- 进一步地，我们如何定义我们的技术要求？我们如何评估 AI 产出的质量？我们如何度量我们的效能水平？我们的人应该重点加强什么能力，应该如何改变我们的组织，等等。
...
总体上，业界整体的态势和预期，是AI软件开发的应用会逐渐深化，AI工具采用会更加普遍，Agent能完成的工作会更加复杂，AI产出的内容占比会更高。

| Timeframe                  | Prediction                                                                                                  | Source                    |
| -------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------- |
| By 2027                    | Over 65% of teams using agentic coding treat the IDE as optional, shifting control to automated platforms   | Gartner                   |
| By 2027                    | Agents can finish month-long coding tasks at the current doubling rate (extrapolated)                   | METR projection           |
| By 2028                    | About 90% of enterprise software engineers use AI code assistants, up from under 14% in early 2024          | Gartner                   |
| Within 5 years (from 2025) | Up to 95% of code could be AI-generated                                                                 | Microsoft CTO Kevin Scott |
| By 2030                    | Engineering roles reorganize around orchestration, system design, and validation rather than implementation | Gartner SE 2030           |


### 现状：组织提效不显性，稳定性下降
而当前，AI 辅助编码在开发者个体层面确实也带来了显著的体感提升。但组织层面的整体提效却仍普遍面临瓶颈。这一现象在业界被称为 **Vibe Coding 生产力悖论**：Agent 代码生成速度呈指数级增长，但组织整体交付效率并未同步跃升。

下图是历史上关于单点技术进步但因为组织方式不匹配而整体效率提升不明显的例子：
Lowell 纺织厂的三次演进：从左至右依次为 1890 年蒸汽动力厂房、1900 年电力驱动厂房，以及 1920 年从零重建的电气化装配线厂房。
19世纪90年代，电力被寄予厚望，人们相信它将带来巨大的生产力飞跃。

新英格兰的纺织厂，原本是为驾驭蒸汽机的旋转动力而建造的，很快便将蒸汽机换成了更快的电动马达。

然而在此后整整三十年里，电气化工厂的产出几乎毫无增长。技术已远超从前，组织却原地踏步。

直到20世纪20年代，工厂们再度对厂房进行彻底重构——引入流水线，为每一台设备配备独立电机，让工人与机器承担截然不同的新职能——电气化才真正带来了实质性的回报。
...
而Faros 2026年的报告看上去更加触目惊心，虽然任务完成量增加了33.7%，史诗级任务（Epics）的交付量更是激增了66.2%，但系统稳定性和质量指标却在更快的速度上走向崩溃。生产环境的缺陷数量跃升了54%，而每合并一个PR引发生产事故的概率更是暴增了242.7%：

| Metric                             | 2025 Faros findings | 2026 Faros findings     | Impact                |
| ---------------------------------- | ------------------- | ----------------------- | --------------------- |
| Tasks completed per developer      | +21%                | +33.7%                  | Positive              |
| Epics completed per developer      | Not measured        | +66.2%                  | Positive              |
| Pull requests merged per developer | +98%                | +16.2%                  | Positive, but slowing |
| Median time in PR review           | +91%                | +441%                   | Bottleneck deepening  |
| Pull request size                  | +154%               | +51.3%                  | Review overload       |
| Bugs per developer                 | +9%                 | +54%                    | Accelerating          |
| Incidents per PR                   | Not measured        | +242.7%                 | Production risk       |
| Organizational delivery            | Flat                | Improving but at a cost | Acceleration Whiplash |

总之，当前AI对软件工程的影响呈现出典型的“局部加速—系统失配”，表现为个体提效明显，但组织收益不显性，同时代码产出加快但系统稳定性下降。
### 一、组织提效不显性的原因
...
今年软研启动会上我曾经引用过一个案例，说明开发者的幻觉，即自己觉得提效明显但实际数据反馈不明显。这个随着大家实践深入应该已经普遍撞过一些墙，这不再是关键问题。但我们必须认识到实际的落地的结果并不是那样乐观。
...
3. **从"人工审查"到"确定性裁判"**：面对概率性软件工程，传统质量保障体系失效。企业必须建立以编译器、自动化测试、基线对比机制为核心的确定性裁判体系，用外部客观验证机制锚定质量，防止 AI 拿"历史遗留"当借口。
...
- 不是把 AI 工具引入了就是 ADLC；
- ADLC 不要过早强调把人从软件交付过程中移除。（国内公司很多人喜欢这么讲，我觉得很危险）

更深一层看，个体 10x 不等于组织 10x。还是前面我们举的1890 年代纺织厂的例子：工厂换上了电动机，但没有重新设计厂房布局和生产流程，导致电气化后 30 年产出几乎零增长——**我们用AI相当于也是换上了"电动机"，但"工厂"还没有重新设计**。
...
首先，我们须认清当前仍处于**个人普及向组织建设过渡**的阶段。**高生产力个体无法自动组成高生产力组织**。我们的重心已经不该是编码本身，而是**组织运转方式**，其中领域专业知识比软件工程专业知识更关键，目标必须从"节省时间"升维到"规模化价值"。

其次，我们应该积极推动组织和人才的转型，逐步构建新的平台、度量、实践三大智能工程支柱。在度量体系上，必须坚决摒弃"AI 生码率"等过程指标陷阱，转向更能指向E2E效率或者质量结果的相关指标，引导开发者保持健康的使用行为。

最后，在架构演进上，应采纳渐进式路径，按需引入上下文增强、质量门禁等，避免过早陷入 Agent 模式的复合错误率陷阱。当然，在模型能力达到临界点前，**半自动人机协作（比如60% 环节使用 AI，核心决策保留人类判断）** 是更具性价比的选择。
...
不过，模型和基础算力等方面的问题更多有赖于采购和推动其他部门解决，不是我们本文讨论的重点。之前反馈较多的模型前端还原度效果较差问题，现在可通过为部分人员开通云SaaS服务账号尝试解决。
...
坏消息是，因为AI推理是一团概率云，所以Agent Loop的方式注定无法收敛。
...
当组织将"AI 生码率"作为考核指标时，会引发严重的"灌水激励"。团队倾向于让 AI 生成大量单元测试、注释、胶水代码等"容易 AI 生成但价值密度极低"的代码。这就是所谓**代码即负债**——代码进入生产即产生维护成本，规模化生产代码等于规模化生产负债。

#####  认知债务与架构漂移

生成式AI本质上是极其强大的模式补全引擎，它们经过训练以生成高度自信、结构严谨且具有说服力的文本。这导致AI生成的代码通常具有极高的“表面正确性”：变量命名规范、代码结构清晰、甚至附带详尽的内联注释说明每一行的意图。但在这层完美的语法外衣下，往往潜藏着深层的架构妥协和业务逻辑缺陷。

而我们人类审查者在面对这种格式完美的代码时，往往会产生心理防线的松懈，倾向于快速批准而不再深入追溯每一行逻辑的来龙去脉（有个词叫”认知投降“）。这种演变是一个温水煮青蛙的过程。在最初的“蜜月期”，AI带来的速度提升令人振奋，团队仍能依靠历史认知控制系统。进入“漂移期”后，机器生成的代码比例反超人类，开发者开始用“大概不会弄坏系统”来安慰自己。最终，当系统演进至“悬崖期”时，一旦发生复杂的生产级宕机，整个团队将陷入无人知晓系统运转机制的恐慌，传统的故障排查演变为盲目的“代码考古”。


尤其在面对我们软件平台产品这样的复杂存量系统时，AI 往往缺乏全局架构感知，导致"按下葫芦浮起瓢"。种种 AI 管理不善、上下文丢失、Agent 行为不可靠所累积的隐性成本，都会形成**认知债务**的累积。
...
##### 垃圾回收式技术债务管理
摒弃传统的"集中清理"模式，采用**垃圾回收式技术债务管理**。将"黄金原则"编码入仓库，部署背景 Agent 定期扫描代码偏离情况、做架构走查、更新质量评级，并自动开启定向重构 PR，持续进行小额度自动化重构，防止冗余代码堆积。另外，考核上尽量避免"AI 生码率"这种虚荣指标（这也是我在研发中心和软研一再反对的，这种指标观察即可）。

##### 基线对比机制与机械强制执行

   - **基线对比机制**：开发前后各跑一次总验证，量化对比指标，客观判定改进或退化，彻底杜绝 AI 拿"历史遗留问题"当借口。
   - **机械强制执行**：通过自定义 linter、结构测试等"品味不变量"，将人类的架构品味编码为机器约束，防止架构漂移。
   - **质量保障四块拼图**：构建 “Rule（软约束）→ Skill（标准化动作）→ Scripts（硬门禁）→ 总验证脚本（统一裁判）”的 Harness 质量保障体系，确保质量收口。
...
假设上游没有定义某种极端异常分支（比如某存储产品和云存储接口类似但缺少关键容错特性，ICC 按照云存储逻辑对接会有丢数据风险）。人类程序员在写代码时会卡住，从而倒逼产品经理澄清需求或者架构师拉通方案；但 AI 会基于概率，自信地编造一个"看似合理但业务错误"的默认处理逻辑（例如直接返回成功或静默失败）。这段代码能跑通，单元测试也能过，但核心业务逻辑全错。由于 AI 生成代码极快，这种"**方向性垃圾**"会被大规模、快速地转化为生产环境中的**代码即负债**（详见前述"AI 生码率陷阱"）。
...
1. 在让 AI 写代码之前，必须先把核心概念、API 契约和业务边界彻底想清楚。如果开发者做不到，可以借助一些spec类的工具帮助拆解问题；
...
大华软件平台深耕行业多年，积累了大量特有的行业术语、业务"黑话"以及特定场景下的"潜规则"。这些知识往往没有写在文档里，而是存在于资深开发者的脑子里，或者隐藏在历史代码的"奇技淫巧"（技术债务）中。假设需求中提到"按照XX行业标准的 XX 协议进行对接"。人类老员工立刻知道这里的"行业标准"特指大华内部针对某类客户魔改过的 V2.3 版本协议，信令、流媒体、智能事件都有各自的特殊要求。但 AI 只能检索到公开的 V1.0 标准协议，从而生成完全无法在真实业务中运行的代码。

当前我们的知识资产极度分散：需求在蜻蜓/TB/Confluence上或者是表格飞来飞去，API 定义在 Swagger/萤火虫，架构讨论在花茶群聊的各种文档和SVN上，代码在 Git。这种**上下文碎片化**导致 AI 只能看到冰山一角。更致命的是，现有的文档是"**给人看的**"，而不是"**给 AI 看的**"。我们的文档中充满了"大家都知道"的省略语、未明说的上下文假设和模糊的自然语言。AI 缺乏人类的"常识脑补"能力，面对这种文档会产生严重的幻觉。
...
##### 能力培养：本质是"投入分摊方式"问题
   ...
   - 设立小规模"赋能团队"（即使是兼职、轮岗形式），专门负责沉淀通用Skill、踩坑经验、最佳实践，降低其他团队的重复学习成本——本质是把分散的学习成本集中化、规模化。软研各团队内部可以明确一些比如上下文工程师，Harness工程师/Loop工程师，飞轮架构师的角色定义，专注于关键资产的沉淀、校验和实践推广。  （比如把团队里最懂业务、最爱写文档的老员工，赋能转型为**上下文工程师**；把团队里手最快的，最懂 CI/CD、最痛恨烂代码、了解DevOps优秀实践的人，转型为 **Harness/Loop 工程师**。把团队里视野最广、关注长期 ROI 的技术专家，赋予**飞轮架构师**的职责。）
   ...
   - AI时代，什么才是真正值得长期培养的核心竞争力？需要大家一起思考，探讨。比如spec编写，验证环节的设计，系统架构的取舍，针对AI交付内容的审查和调试。
...
另外，虽然我们大华过去的系统多、工具多，但现在这个问题可能不再显得那么严重了——这些系统可以快速封装为 MCP Server，以命令行形式供 Agent 调用，从而实现更高效更集中地一站式复用现有资产。
...
- **绿地/简单场景**:提效35%-40%,可作为AI能力上限的参考基准。比如我们全新开发的定制服务，新的探索类业务；
- **遗留/复杂场景**:受上下文碎片化、技术债拖累,提效约10%,这才是大多数存量项目的真实水平。比如VSL等历史悠久的遗留系统；
- 同时也要考虑转型学费期的效率下降是否在预期之内
...
当前务实状态是"60%环节用AI、完全委托Agent仅0%-20%"，半自动协作是主流且更具性价比。这意味着反馈二中的最小公共指标集,不能只衡量"AI做了多少",还要衡量**协作结构本身**:
...
参考：开发者AI影响力框架（Developer AI Impact Framework）

| **影响力框架支柱**           | **核心度量指标定义**                              | **深度解析与业务价值关联**                                            |
| --------------------- | ----------------------------------------- | ---------------------------------------------------------- |
| **1. AI 采用与参与度**      | 周活跃用户率 (WAU)、工具模式分布 (内联补全 vs. 对话 vs. 智能体) | 衡量工具是否真正在日常中沉淀。低于30%的WAU通常意味着培训不足或工具与工作流严重脱节。              |
| **2. 代码贡献占比**         | AI辅助PR比例、AI生成的代码行占比                       | 揭示代码库中机器逻辑的渗透深度。占比过高（如>40%）若无质量制衡，则预示着系统风险的急剧累积。           |
| **3. 复杂性调整吞吐量 (CAT)** | 基于架构难度的产出计分 (简单=1, 中等=3, 困难=8)            | 彻底消除“刷代码行数”的度量幻觉。确保评估的是实质性的功能交付，而非无脑生成的样板文件。               |
| **4. 代码生命周期与质量**      | 代码流失率 (Code Turnover, 30天/90天)、创新率比值      | 核心质量防线。精准追踪AI代码在部署后被重写或废弃的比例。精英团队将AI代码流失率严格控制在人类基准的1.3倍以内。 |
| **5. 成本与净投资回报率**      | 综合AI工具成本、节约时间价值、因返工造成的价值损耗                | 财务维度的终极校验。将工具许可、算力消耗与重构成本相加，对比实际交付提升，确保AI投资具备可持续的商业逻辑。     |

此外，反馈二中"最小公共指标集"的核心新增指标应是**Token效率比**(完成一个有效任务的平均Token消耗),取代失效且易被"灌水"的代码行数/PR数量:
...
结合前述内容，为了更好地评估软件平台组织的AI原生开发能力，并在今后的1-3年内持续牵引团队提升ADLC的端到端质效水平，我们尝试构建一个成熟度模型。该模型可以分成以下几个维度：

- **基础设施**（I for Infrastructure）：包括算力服务器资源，模型选型和管理，资源调度和分配；
- **知识工程**（K for Knowledge）面向AI准备的知识工程体系；
- **流程引擎**（E for Engine）人和AI协作的开发流程和对应工具。本质是构建AI时代的软件生产流水线。
- **评估治理**（E for Evaluation）Agent开发的质量/安全/合规/可用性等各方面的评估治理体系。
- **组织人才**（P for People）不断探索并演化新的开发组织形式，刷新人的能力成长要求。

考虑到我们软件平台不会花太多精力在 Infra 这一层，我最终只取了后四个维度，刚好可以简称为 **KEEP 模型**。该模型重点在于评估组织能力，而非单体个体开发者的能力水平（我们和人力资源在同步刷新应用软件工程师的任职资格，更适用于个人能力的衡量）。
...
| 阶段                               | 阶段目标   | 核心特征                                                                                                                                       |
| -------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **L1** 辅助提效（AI Assisted）         | 释放个体潜能 | AI 作为"更好的工具"被个体开发者采用，重复性工作得以自动化，对于系统知识、代码仓遗留系统的学习曲线缩短，个体生产力有显著提升——但 AI 的使用仍停留在个人或极小团队层面的零散尝试。                                              |
| **L2** 领域集成（Domain Integrated）   | 构建组织智能 | AI 能力从个人工具升级为组织基础设施，系统化集成至特定研发领域和流程。领域知识沉淀为可复用资产，团队级能力标准建立，AI 投入开始在 production 环节产生可衡量的组织效益（比如端到端的需求吞吐）。                               |
| **L3** Agent 协同（Agentic Synergy） | 倍增协作效能 | 人机协同模式正式确立，Agent 承担明确的业务交付角色。每位工程师通过 Agent 协同能覆盖更大范围的工作，人力杠杆效应显现，团队有效产能显著超越传统模式。这一阶段的收益（效率或者质量方面）应该能让更高层面的组织比如研发中心或者外部门比如市场组织有比较清晰的感知。 |
| **L4** 自主代理（Autonomous Agents）   | 实现自主交付 | Agent 能端到端承担 ADLC 六个并发阶段的交付任务，知识飞轮驱动经验自动沉淀与进化。人从绝大部分的直接执行中解放，转向战略定向、异常裁决和 Harness 治理。此时AI完成的工作更像是一条完整的流水线。                             |
...
| --------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| **知识工程**<br>（K） | · 文档未结构化，人工检索为主，且未面向AI准备<br>· 无工程化知识管理手段                        | · 有相对比较完善的知识库或者文档库建设；<br>· 包括All in Code在内的Context Engineering 实践启动；<br>· 有 AI 案例库沉淀 | · 跨领域知识体系构建；<br>· 领域 spec/rule/Skill 库与质量管理体系较为完善。                                          | · 知识飞轮：Agent自动完成知识的更新和lint；<br>· Agent具备跨领域知识，可以贯穿支撑跨团队开发和问题定位等工作；<br>· 知识的自优化和自动沉淀，自主识别盲区并自我扩展                                                    |
| **流程引擎**<br>（E） | · AI 辅助工具作为研发各环节的补充；<br><br>· Vibe Coding 探索式使用                 | · AI 集成 到DevOps 关键环节，可独立完成特性级别的开发到验证完成。<br>· 实现智能代码审查与测试生成<br>                       | · 自适应的流程规范（Spec 驱动）<br>· 统一配置/最佳实践管理和AI效能看板<br><br>                                         | · AgentOps 与 VPT 效能度量驱动持续优化<br>· Agent 具备自主设计与规划能力<br>· 多 Agent 自主编排协同与 Token 优化<br>· 飞轮自我进化与配置盲区自动发现<br>· 自进化飞轮基础设施：自动化评估与在线优化反馈回路<br>· 成本与效益智能优化 |
| **评估治理**<br>（E） | · AI 内容人工审核<br>· 简单日志记录，缺系统风险评估                                 | · 明确AI 完成工作的检查要求，比如UT覆盖率；<br>· 使用AI 进行代码安全扫描与合规检查                                    | · 明确Agent 权限边界定义与强制执行要求<br>· 明确Agent 执行质量验收标准<br>· Agent 行为完整可追溯审计,包括质量、效率结果分析能力，构建半自动化改进回路 | · 团队级认知债务评估与管控<br>· 实现Agent 自优化安全约束与自动回滚<br>· 实现AI 可信度评级与影响评估<br>· 构建事故的AI自学习回路，实现人机共治（人保留最终控制权）                                                   |
| **组织人才**<br>（P） | · 传统组织加 AI 工具培训<br>· 基本 Prompt Engineering 技能<br>· 个人探索，无系统能力建设 | · AI CoE 推广实践与培训体系<br>· 开发者学习 Context Engineering<br>· AI 使用效率纳入度量                   | · 角色转型：Loop Engineer 等<br>· 开发者转型为 Agent 协作监督者<br>· 人机协同工作规范与评估标准                           | · Agent Orchestrator/Evaluator 新岗<br>· 组织运转接近自动化软件工厂<br>· 基于价值创造与 Token 效率考核<br>· 飞轮架构师：飞轮的设计者与守护者<br>· 创新孵化平台：由超级个体 + 多 Agent 组成的完整生产与创新团队        |


怎么判断领域，举例说明：
- 团队缺少agent相关技术规范算是流程引擎相关领域的问题，因为这是定义工作流水线、开发流程的要素。
- 需求设计质量问题较多可以算是知识工程的问题。因为很多是缺少有效的行业、工程的上下文。但也和流程引擎问题有相关，比如可能需要借助流程引擎的spec的定义和管理来约束到每个团队个人的执行过程。
- 建立CoE是组织人才领域的问题。
...
| ------ | ------------------------ | --------------------------------------------------------------------- | --- |
| 知识整合   | 从仅使用公开知识 → 领域知识库建设启动     | 各领域已建立 蜻蜓(Confluence) /SVN/Git知识库，行业知识有过结构化整理，比如协议统一管理。               | 1.5 |
| 文档结构化  | 从人工检索为主 → 开始探索 AI 友好格式   | 部分团队试点 AGENTS.md（或CLAUDE.md,CODEBUDDY.md），文档开始考虑 AI 可读性。有repo wiki实践。 | 1.3 |
| 知识管理手段 | 从无工程化手段 → LLM-Wiki 方案设计中 | 有一些LLM-Wiki 概念验证完成，有RAG实践经验。                                          | 1.5 |

**已有成果**：
- 部分产品线建立了行业知识库
- 开始推行 AGENTS.md 和 architecture.md 实践
- LLM-Wiki 等技术方案已完成 POC

**待补差距**：
- 行业特有知识（术语、"黑话"、"潜规则"）显式化程度不足
- 知识资产仍分散
- 缺少统一的 All In Code 实践规范指导，渐进式信息暴露机制尚未建立

**年度目标**：L2 稳定 → L3 起步（建立跨领域知识体系，完善包括Skill 在内的质量管理体系，规范All In Code 实践）
...
| 评估项     | L1→L2 进展                      | 当前状态                                        | 得分  |
| ------- | ----------------------------- | ------------------------------------------- | --- |
| AI 工具嵌入 | 从可选工具 → DevOps 关键环节集成         | Codebuddy 深度集成，慧码 spec等工具覆盖代码审查、测试生成        | 2.0 |
| 流程主导    | 从人主导 → AI 参与流程编排              | 部分团队试点实现 Spec 驱动，试点项目尝试agent自主完成工作。         | 1.7 |
| 探索程度    | 从 Vibe Coding → 领域 Harness 实践 | 尚未有 完善的Harness Engineering 概念，但部分团队有落地rule， | 1.8 |
...
| 能力建设 | 从个人探索 → 系统化能力建设规划 | 新角色定义（上下文工程师、Loop工程师）讨论中，能力模型设计中  | 1.4 |
...
- 全栈（或者"半栈"，总之是尽可能覆盖更多技能或环节，更合适的定义可能叫“AI角色连续体（AI Roles Continuum）”）能力的组织重构需试点推进
- AI 使用效率需纳入正式度量体系
...
| 行动项                                                | 对应维度 | 预期效果                            |
| -------------------------------------------------- | ---- | ------------------------------- |
| 发布知识工程相关规范，盘点现有知识资产，建立 LLM-Wiki 知识库，推行 All In Code | 知识工程 | 解决上下文碎片化，为其他建设打基础               |
| 部署确定性裁判体系（hooks + 自动化测试完备）                         | 评估治理 | 降低隐性验证税，保障质量底线                  |
| 建立 Skill 开发规范，完善评审和管理机制                            | 流程引擎 | 统一工具链，促进复用                      |
| 启动 AI CoE，正式定义研发团队新角色                              | 组织人才 | 系统化能力建设                         |
| 探讨Token 效率比度量，质效看板上线                               | 流程引擎 | 数据驱动持续优化                        |
| 推行 全栈组织重构试点                                        | 组织人才 | 提升小团队作战能力                       |
| 领域 Harness 治理规范和工程实践落地                             | 评估治理 | 政府、企业、基础、海外、西安、成都等各团队均落地自适应流程规范 |
...
我们期望在今年（2026年），软研的成熟度整体达到2级，部分试点项目的个别探索方向达到3级；到2027年能够整体达到3级。

### 大华软件平台AI原生开发体系总体说明

整个体系由六个核心组件构成，覆盖从”谁来定义方向”到”谁来执行落地”、从”标准规范”到”度量反馈”的完整闭环。以下逐一展开，并重点说明组件之间的数据流和协作关系。

#### One Group（CoE）— 技术与能力中枢

定位：以 **CoE（Center of Excellence）为核心的技术虚拟组织**，为技术标准规范和先进生产力方向负责。

构成：
- **核心成员**：架构师组织 + 各团队 TL + 一线 AI 原生开发新生力量
- **外围接口人**：各领域的上下文工程师、Harness 工程师作为 CoE 与团队的连接点

职责：
- 定义和维护 **One Spec**（技术规范体系），确保全组织遵循统一标准
- 运营 **慧码社区**（内部开源），推动工具和组件的孵化与共享
- 组织能力培养与最佳实践推广（培训、技术沙龙、黑客松）
- 定期评估 KEEP 成熟度模型，牵引整体能力升级

数据流向：CoE 产生的 Spec 是其核心输出物，通过 **One Runtime** 下发到各团队开发环境中执行；执行效果反馈由 **One DashBoard** 汇聚后驱动 CoE 迭代方向。

#### One Spec（标准与规范体系）

定位：标准化研发与交付行为，提供统一工程与质量约束。

Spec 覆盖三大领域：
- **知识工程规范**：All in Code 规范、代码仓上下文结构标准、LLM-Wiki 知识库建设指南
- **流程引擎规范**：Skill/Rule/MCP 开发规范、Agent Runtime 接入标准、工具数据上报协议
- **评估治理规范**：AI 质量门禁标准、UT 覆盖率阈值、安全合规检查规则、Token 效率比度量方法

运行方式：
- By CoE（由能力中心制定和维护）
- For All（面向全组织发布和执行）
- Under KEEP Model（以成熟度模型为指引，逐级提升要求）

闭环机制：Spec 不是静态文档。**One Runtime** 将其注入实际开发流程，执行结果通过 **One DashBoard** 反馈回 CoE，CoE 根据实际效果迭代 Spec——形成”制定→注入→度量→迭代”的持续演进回路。

#### One DashBoard（统一看板）

定位：集成算力、项目、开发过程、质量、技术债务等关键度量的看板体系，为经营决策和质效提升提供数据支撑。

看板分层：
1. **算力层**：基础算力利用率、Token 消耗总量、API 调用并发及时延
2. **业务层**：各产品线的 AI 工具使用情况、组织/个人的 Token 消耗分布
3. **资产层**：Skill/MCP/Rule/Agent 的分布、活跃度、质量评级（数据来源：**One Market**）
4. **效能层**：交付效率指标（需求吞吐、PR 合并速度、缺陷率）、Token 效率比

数据来源关系：
- **One Runtime** 采集的端侧行为数据是核心输入（任务类型、Token 消耗、采纳/回退率）
- 存量系统补充（TB项目数据、萤火虫资产数据、IT算力平台、WeTrack 缺陷数据）
- **One Market** 提供资产质量评级数据

使用方式：DashBoard 嵌入 **One Workplace** 中，团队管理者在同一界面可同时看到”执行情况”和”度量结果”。

#### One Workplace（统一工作空间）

定位：人 + 项目 + 机器的协同环境，是团队管理的驾驶舱和软件生产流水线的操作台。

核心能力：
- **项目管理**：团队任务、排期、进度一览
- **人员协作**：团队成员分工与角色标定（上下文工程师、Harness 工程师等）
- **人机协同执行**：人 / AI Agent 作为数字员工在同一空间协作
- **策略下发**：CoE 的规范要求、最佳实践、核心资产（Agent、Skill 等）推送至团队

扩展关系：
- 内嵌 **One DashBoard** 视图，实现”执行”与”度量”同屏
- 与 **One Runtime** 形成执行闭环：在 Workplace 中启动开发任务，Runtime 负责执行和采集数据
- 集成 **One Market**：在 Workplace 中可直接搜索、安装、评价市场中的资产

#### One Runtime（慧码 Agent Runtime）

定位：执行引擎层，是规范落地和度量采集的核心单元。

核心能力：
1. **规范注入**：将 One Spec 中的规则以 Hook 形式插入 Agent 执行流程，在关键节点（commit 前、PR 时、部署前）触发质量门禁
2. **度量采集**：自动记录 Token 消耗、工具调用链、任务耗时、采纳/回退比例，解决端侧可观测性盲区
3. **外部系统对接**：以 MCP 协议链接萤火虫（组件资产）、TB（项目协作）、WeTrack（缺陷管理）等存量系统，统一 Agent 访问入口
4. **Harness 适配层**：在未来支持多种 Code Agent（Claude Code、Cursor、CodeBuddy 等）的背景下，Runtime 承担通用 Agent 之上的定制化逻辑

数据流方向：
- **上行**：采集的行为数据 → 按统一协议上报 → 汇入 **One DashBoard**
- **下行**：从 **One Market** 获取最新 Skill/Rule 资产 → 注入 Agent 执行环境

#### One Market（统一资产市场）

定位：面向全组织开放的数字资产流通平台，提供 Agent、Skill、MCP、Rule、Spec 等关键资产的发布、发现、评价和复用机制。

核心机制：
- **准入机制**：所有资产必须遵循 **Skill 开发规范**（由 One Spec 定义），通过自动化评估回归测试方可发布
- **评级体系**：基于使用频次、用户评分、质量检测结果进行多维度评价，区分高活跃资产、僵尸资产和劣质资产，在市场中晾晒
- **生命周期联动**：与”慧码社区”的沙盒→孵化→核心→退役四阶段机制联动，明确每件资产的成熟度和维护承诺

市场价值：
- 消除”各团队各自为战、重复造轮子”
- 降低优秀实践的复制成本——A 团队的 Skill 成果可直接被 B 团队安装使用
- 为 CoE 提供资产健康度的全景视图，驱动治理决策
...
六个组件的关系可以概括为三条流和一个整合层：

**规范流**：One Group（制定）→ One Spec（定义）→ One Runtime（注入执行）
**数据流**：One Runtime（采集）→ One DashBoard（呈现）→ One Workplace（消费/决策）→ 反馈至 One Group
**资产流**：各团队和 One Runtime（创造）→ One Market（沉淀复用）→ 各团队通过 One Workplace 发现和安装
**整合层**：One Workplace 是前台的统一入口，内嵌 DashBoard 视图、对接 Market、触发 Runtime 任务。

具体来说：
- **One Group** 看护一套技术规范（One Spec），
- **One Spec** 通过 **One Runtime** 注入到工程执行中，
- **One Runtime** 在支持开发执行的同时采集标准化度量数据，输出到 **One DashBoard**；
- **One Workplace** 作为统一工作入口，将 DashBoard 的度量和 Market 的资产整合呈现给团队；
- 优秀的实践成果（Skill/Agent/Rule）沉淀到 **One Market**，
- **One DashBoard** 的数据和 **One Market** 的资产健康度反过来驱动 **One Group** 迭代规范。

整个体系的原则是**开放但有序**：各自的工具建设可以有，但不必要的、有冲突的不做；有必要做的必须统一接口标准和度量收口。通过这套体系，鼓励开源共创、集体维护，让每个团队既是规范的执行者，也是资产的贡献者。
...
结合着任职、新角色探讨，虚拟组织构建，和人力资源一起讨论后续行动。

#### CoE虚拟组织
建立CoE虚拟组织。除了王葛亮，架构师组织和原有团队TL之外，重点吸纳一线AI原生开发的新生力量。

#### 人才认知和新角色定义
明确以下几类人才定义，和我们的任职资格结合起来：
...
整个引擎体系其实核心就三个东西：
- one board 一个统一效能看板呈现我们的AI原生开发质效详情
- one runtime 一个标准agent运行时构建我们的harness端侧
- one market 一个统一AI资产市场归集我们的AI开发数字资产
...
#### One Workspace：人机协作平台


目前AI原生开发更多还是关注个人的提效，在开源社区，小型创业团队中有较多优秀的实践。但是当我们真的进入到ADLC的时代，针对大型企业级软件的开发，如何确保团队的优秀实践的落地，管理所有人和AI的行为，针对效能和质量有更有效的观察和提升？

为此我们提出人机协作平台，针对团队级别的AI原生开发，提供一站式的流程编排、规约和质量要求下发、度量汇聚分析、团队和AI协作能力：

- 对人员、Agent进行统一管理
- 从Market拉取所需要的数字资产
- Context Engineer同步知识库等相关信息到空间，进行维护和管理
- 由Loop Engineer统一编排工作流程，下发团队最佳实践约束
- Loop Engineer 配置背景Agent，用于质量、安全等维度的扫描
- 从Runtime 和其他系统采集所需要的数据，形成人员和项目维度的效能和质量数据
- 向Board提供数据，进行经营，效能质量管理方面的多维度数据洞察；
- 从Runtime接受个人优秀实践上传，实现个人到组织的复制；
- 把资产、流程、质量校验规则和背景Agent打包分发到其他项目，完成跨项目优秀实践分享，实现组织到组织的复制；
...
关于harness分层逻辑：我们使用的通用code agent可能在未来会有多种，内部开发和外部各种厂商皆有可能。所以我们的runtime重心应该放在通用之外，我们更有自身特色的部分。
![[Pasted image 20260620202242.png]]
...
我想的解决方式是： 1）开源社区方式运作，明确生命周期和退出机制；2）做好奖励和分享。我去年讲过慧码应该是个社区，在今年AI工具普及之后，我们的开发如果有好的想法其实可以鼓励他们更多投入进来，成为Everything Builders！
...
AI原生软件开发的评估治理体系,目标是让"AI生成的代码和过程是否可信、是否达标"变得可衡量、可追溯、可持续改进,而不是停留在人工抽查和事后补救。这部分工作将以一系列规范文档为产出主线，7月底完成初稿，后续在试点中迭代。
...
- **集成前 / commit 前**（快、便宜）：Linter、快速测试、基础代码审查 Agent
- **集成后 pipeline 中**（更贵、更全面）：变异测试、全景代码审查（重复快速检查 + 新增重量级检查）
- **持续漂移传感器**（生命周期外持续运行）：死代码检测、测试覆盖质量分析、依赖扫描、SLO 退化监控、日志异常检测

### 下一阶段关键工作和里程碑



| 内容                              | 分类   | 输出结果                                                                                      | 优先级 | 负责人        | 完成时间                 |
| ------------------------------- | ---- | ----------------------------------------------------------------------------------------- | --- | ---------- | -------------------- |
| One Runtime 慧码 Agent Runtime V1 | 流程引擎 | Runtime + Hook 机制上线；实现 spec 规范注入、度量数据采集、外部系统对接                                            | 高   | 公共         | 7月首个版本，发布CLI和插件。持续迭代 |
| One Board 效能看板 V1 上线            | 流程引擎 | 整合算力、Token、资产、效能数据的统一看板；支持组织下钻到个人                                                         | 高   | 公共         | 7月首个版本，持续迭代          |
| One Spec：Huima Spec             | 流程引擎 | 参考OpenSpec，发布大华慧码spec                                                                     | 高   | 公共         | 7月首个版本，持续迭代          |
| Skill 开发规范与 管理机制                | 流程引擎 | Skill 开发规范文档；Skill/MCP/Rule 统一市场；含评价评估体系和推荐机制；含安全合规扫描等准入机制                                | 高   | 公共         | 已部分完成，7月正式运行         |
| 启动 AI CoE 虚拟组织,新角色定义与任职资格对接     | 组织人才 | CoE 组建完成（架构师+TL+一线新生力量）；运作机制确定;上下文工程师、Harness工程师、飞轮架构师角色定义文档；与 HR 任职资格对齐                  | 高   | 人力、架构、所有团队 | 7月                   |
| 黑客松大赛                           | 组织人才 | 黑客松举办并完成评优                                                                                | 高   | 人力、架构      | 7月                   |
| "慧码"社区化运作启动                     | 流程引擎 | 公共仓库建立；准入标准、角色定义、生命周期机制发布；首批项目入驻                                                          | 中   | 人力、架构、所有团队 | 7月-8月                |
| 发布《软件平台知识管理代码化下沉》规范             | 知识工程 | All in Code规范文档；各团队完成至少1个代码仓的 All in Code 试点。                                             | 高   | 架构、所有团队    | 7月中刷新规范，7月各团队闭环      |
| 大华软件平台AI原生开发质量门禁规约              | 评估治理 | 代码 Review Agent / 安全扫描 Agent 接入规范；UT覆盖率阈值定义；Runtime度量采集方案；明确各种检查手段，包括强制/非强制说明，AI或脚本方式说明等。 | 高   | CoE        | 7月底初稿；8月正式版          |
| LLM-Wiki 团队内试用与迭代               | 知识工程 | 试用反馈报告；确定正式建设方案                                                                           | 中   | 架构         | 8月                   |
| 测试团队协作模式探讨                      | 组织人才 | 测试左移/右移试点方案；开发-测试协作流程变更建议文档                                                               | 中   | PQA、人力、架构  | 8月                   |
| 架构治理背景Agent上线                   | 评估治理 | 跨业务代码扫描，安全合规，专项架构审查等。                                                                     | 中   | 公共/架构      | 8月                   |
| AI质量门禁试点                        | 评估治理 | 所有试点项目完成 Loop AI 质量门禁标准                                                                   | 高   | 所有团队       | 9月                   |
| 《大华软件平台AI原生开发技术白皮书2026》         | 综合   | 白皮书发布，涵盖 KEEP 成熟度模型、建设成果、技术方向                                                             | 高   | 郑艳涛        | 10月                  |
...
如前所述，无论大华内部还是行业，AI在开发领域的应用已经是普遍的事实，对于质量的保障无论从效果上还是速度上，都已经变成一个紧迫的现实。与此同时，测试团队面临的不仅是工作量的压力，更是角色存续的根本挑战。仅仅精通点击界面和维护Selenium脚本的传统手工或初级自动化测试人员，会有比较现实的压力。

当前大华的测试岗位无论从工作流程上还是技能要求上尚未有明确的外显的变化。因为传统的IPD过程更倾向于细化分工、客观上有增加开发和测试在流程环节上的屏障，而在ADLC里这一定会变成一种显式的瓶颈。我先尝试做一些分析，希望能够未来有更多实质性的探讨：
##### 岗位左移与测试左移/右移

**岗位左移**：测试人员的角色从"执行测试"向"质量设计"迁移。测试人员不再驻守在开发流程的末端，而是前移到需求澄清、spec 设计、验收标准定义等环节。这与 Harness Engineering 中"前馈控制（Feedforward Guides）"的思路一致——质量防线越早建立，纠错成本越低。

**测试左移**：测试活动和验证逻辑嵌入到开发的最早阶段。测试团队参与 spec 评审，将验收标准编码为可执行的验证规则（Scripts / 结构测试），在 Agent 编写代码之前就完成测试策略设计。这呼应了前述"确定性裁判"体系中"质量左移"的原则。

**测试右移**：验证从"开发完成后的独立阶段"延伸到生产环境。通过可观测性（SLO 监控、日志异常检测）、线上验证和生产环境中的持续漂移传感器，测试成为贯穿交付全链路的持续活动。

##### AI 原生范式下的协作模式

AI原生范式不再容忍开发、测试和运维之间的孤岛。当 Agent Loop 加速了交付循环，传统的瀑布式流水线协作必须被打破，长期看测试不应该再是开发之后的独立环节，而是内嵌于 Loop 的持续质量活动。

关键变化：
- **从"交接"到"对齐"**：开发和测试不再各自独立完成交付物再"丢过墙"，而是在 spec 设计阶段就对齐全局验收标准
- **从"事后拦截"到"事前防御"**：测试的价值从"发现问题"转向"定义什么是对的"，通过验证 Agent 和门禁规则在 Loop 内完成大部分检查
- **从"独立阶段"到"内嵌活动"**：测试活动不再是时间线上的一个节点，而是融入 Agent Loop 的持续验证

后续需要和测试团队继续探讨：
- 协作模式变更的具体流程：从需求到发布的完整流程中，测试角色在哪些环节介入、与开发如何协作。
- 以前开发和测试是泾渭分明的分工。当"Loop"真正到来，循环加快，本来理论上可以快速完成自愈和自修复——但因为惯性还是在机械地拦截、驳回、返工，这本身就是显著瓶颈。
- 如果开发和测试的工作还有很大隔离，各自有 AI 协助完成各自的交付物，那该在哪个环节对齐？出现争议时如何判定？
- 如何度量测试团队转型后的价值？传统指标（用例数、缺陷发现数）是否失效？新的度量标准是什么？

##### 行动方向

**反哺 Loop**：对于 AI 生成的成果，如果下游测试活动发现问题，应该思考的是如何前置把验证 Agent 加强——将测试反馈编码为 Loop 内的 Sensor，反哺加入自纠正过程，而不是停留在事后拦截。

**目标协作场景**：一个原先的开发人员和一个原先的测试人员接到同一个需求，他们讨论之后，一起结对设计了相关的 spec，下发指令给了 Agent 进行开发，对于缺少的用例也给出了方向。第二天早上，他们一起看了所有的执行结果，review 了代码，看了用例的修改情况，审视了验证报告。他们认为版本可以发布了。

这一场景需要：
- 测试人员具备 spec 设计和验证规则编码的能力
- 开发人员具备测试思维，在 spec 阶段就考虑边界和异常
- 共享的验证 Agent 和门禁体系作为双方协作的"公共语言"
...
去年（2025年）我们讲软件工程化专项，提的是三大效能支柱；今年我们启动组织级AI原生开发能力建设，构建的是KEEP四维度的成熟度模型。它们之间不是替代关系，而是一种延续：
![[Pasted image 20260614211104.png]]

AI 软件工程的本质，是从"管理代码生产"走向"管理智能生产"。如今产出不再稀缺，稀缺的是确定性。在AI原生开发的浪潮中，最终开发者们一定会形成共识：真正贵的不是 Token，而是失控。我们必须正视认知陷阱，构建适配自身的上下文工程与确定性裁判体系，才能在 AI 浪潮中将大华软件平台研发的个体生产力真正转化为组织级的竞争壁垒。
