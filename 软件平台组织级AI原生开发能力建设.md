...
- 我们后续的行动是什么？我们如何定义我们的技术要求？我们如何定义我们的建设方向？我们如何评估 AI 产出的质量？我们如何度量我们的效能水平？我们的人应该重点加强什么能力，应该如何改变我们的组织，等等。
...
Faros 2026年的报告看上去更加触目惊心，虽然任务完成量增加了33.7%，史诗级任务（Epics）的交付量更是激增了66.2%，但系统稳定性和质量指标却在更快的速度上走向崩溃。生产环境的缺陷数量跃升了54%，而每合并一个PR引发生产事故的概率更是暴增了242.7%：
...
更深一层看，个体 10x 不等于组织 10x。前面我们举的1890 年代纺织厂的例子：工厂换上了电动机，但没有重新设计厂房布局和生产流程，导致电气化后 30 年产出几乎零增长——**我们用AI相当于也是换上了"电动机"，但"工厂"还没有重新设计**。组织级智能不会随个体使用 AI 工具自然涌现，它需要在**协调、信号提取、确定性保障**三个支柱上构建制度体系。这意味着：**流程工程（Process Engineering）** 才是核心——不是编码产品，而是把企业流程编码进 Agent 并推动组织变革管理，其中**领域专业知识比软件工程专业知识更关键**。目标必须从"节省时间"升维到"规模化价值"——否则"AI 生码率"陷阱不可避免。正如业界所言：问任何 CEO 首要目标是降本还是增收，几乎所有人都会说是收入；但几乎所有 AI 产品都在卖"省时、少人"——这中间的鸿沟，就是组织级能力建设的意义所在。


而总体上，业界整体的预期，是AI软件开发的应用会逐渐深化，AI工具采用会更加普遍，Agent能完成的工作会更加复杂，AI产出的内容占比会更高。我判断这种前景普遍乐观和落地存在焦虑的情况会持续并存一段时间。
...
| --------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| **知识工程**<br>（K） | · 文档未结构化，人工检索为主，且未面向AI准备<br>· 无工程化知识管理手段                        | · 有相对比较完善的知识库或者文档库建设；<br>· 包括All iin Code在内的Context Engineering 实践启动；<br>· 有 AI 案例库沉淀 | · 跨领域知识体系构建；<br>· 领域 spec/rule/Skill 库与质量管理体系较为完善。                                          | · 知识飞轮：Agent自动完成知识的更新和lint；<br>· Agent具备跨领域知识，可以贯穿支撑跨团队开发和问题定位等工作；<br>· 知识的自优化和自动沉淀，自主识别盲区并自我扩展                                                    |
...
我想的解决方式是： 1）开源社区方式运作，明确生命周期和退出机制；2）做好奖励和分享。我去年讲过慧码应该是个社区，在今年这个时刻，我觉得是时候了。让我们成为Everything Builders！
...
- **持续漂移传感器**（生命周期外持续运行）：死代码检测(也许在这个AI时代，我们更容易接受这个现实：代码即负债）、测试覆盖质量分析、依赖扫描、SLO 退化监控、日志异常检测
